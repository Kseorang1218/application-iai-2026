...
P3(EnvSpec): magnitude 추출 → (선택적 log1p) → per-window z-score.
P4(Cepstrum): raw window에 per-window z-score 적용 후 feature 추출.
P6(TDS): raw window에 feature 추출 후 per-window z-score 적용.
...
def make_feature_fns(
    ...
    env_spec / order_spec: magnitude 추출 → (log1p=True면 log1p 압축) → per-window z-score.
    ...
    log1p : bool
        True면 env_spec/order_spec magnitude에 log1p 압축 적용 후 z-score.
